...
def grille_recherche_hdbscan(embeddings, labels, param_grid, n_samples=None, checkpoint_file="resultats/optimisation_checkpoint.pkl"):
    ...
    for i, params in enumerate(tqdm(combinaisons_restantes)):
        ...
        if n_clusters > 1:
            mask = clusters != -1
            # Correction: s'assurer qu'il y a au moins 2 points par cluster
...
def visualiser_resultats_optimisation(resultats_tries):
    ...
    sns.barplot(x=list(range(max_configs)), y=silhouettes[:max_configs], ax=axes[0])
    ...
    sns.barplot(x=list(range(max_configs)), y=n_clusters[:max_configs], ax=axes[1])
    ...
    sns.barplot(x=list(range(max_configs)), y=percent_noise[:max_configs], ax=axes[2])
...
def visualiser_composition_clusters(composition, n_clusters):
    ...
    for i, bar in enumerate(bars.patches):
        if i < len(df_viz):  # Vérifier l'indice pour éviter les erreurs
...
def clustering_deux_niveaux(embeddings, df_fiable, params_hdbscan):
    ...
    for cause in causes_uniques:
        ...
        indices_cause = np.where(mask_cause)[0]  # Indices relatifs au DataFrame courant
...
def predire_causes(modele, embeddings):
    ...
    try:
        # Correction: prédire sur l'objet clusterer
        clusters_niveau1, strengths = clusterer_niveau1.approximate_predict(embeddings)
    except Exception as e:
        print(f"Erreur avec approximate_predict: {e}")
        try:
            # Correction: prédire sur l'objet clusterer
            clusters_niveau1, strengths = clusterer_niveau1.predict(embeddings)
        except Exception as e:
            print(f"Erreur avec predict: {e}")
            # Si les deux méthodes échouent, utiliser une approche par KNN
            from sklearn.metrics import pairwise_distances
            print("Utilisation de l'approche KNN pour la prédiction...")
            ...
            for emb in tqdm(embeddings, desc="Prédiction KNN"):
                # Calculer les distances à tous les points d'entraînement
                distances = pairwise_distances([emb], original_data, metric='euclidean')[0]
                
                # Vérifier si le tableau des distances est vide
                if len(distances) == 0:
                    cluster_id = -1
                    continue
                
                # Trouver les k plus proches voisins
                k = min(5, len(distances))  # Ajuster k si nécessaire
                nearest_indices = np.argsort(distances)[:k]
                
                # Vérifier si nearest_indices est vide
    ...
    if not isinstance(clusters_niveau1, list):
    ...
    for i, cluster_id in enumerate(clusters_niveau1):
        ...
        if cause in clusterers_niveau2 and cause != "Non déterminée":
            ...
            try:
                # Prédire avec le modèle de niveau 2
                cluster_niveau2, _ = clusterer_niveau2.approximate_predict([embeddings[i]])
                cluster_niveau2 = cluster_niveau2[0]  # Extraire le seul élément du tableau
            except Exception as e:
                try:
                    cluster_niveau2, _ = clusterer_niveau2.predict([embeddings[i]])
